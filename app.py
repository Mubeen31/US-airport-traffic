...
airport = pd.read_csv('2011_february_us_airport_traffic.csv')
...
def update_graph(select_state):
    airport4 = airport.groupby(['state', 'airport', 'city', 'lat', 'long'])['cnt'].sum().reset_index()
    ...
    return {
        'data': [go.Scattermapbox(
            ...
            hoverinfo = 'text',
            hovertext =
            '<b>State</b>: ' + airport5['state'].astype(str) + '<br>' +
            ...
            '<b>Arrivals</b>: ' + [f'{x:,.0f}' for x in airport5['cnt']] + '<br>'
...
@app.callback(Output('bar_chart', 'figure'),
              [Input('select_state', 'value')])
def update_graph(select_state):
    airport2 = airport.groupby(['state', 'airport', 'city', 'lat', 'long'])['cnt'].sum().reset_index()
    ...
    return {
        'data': [go.Bar(
            ...
            hoverinfo = 'text',
            hovertext =
            '<b>State</b>: ' + airport3['state'].astype(str) + '<br>' +
            '<b>City</b>: ' + airport3['city'].astype(str) + '<br>' +
            '<b>Airport</b>: ' + airport3['airport'].astype(str) + '<br>' +
            '<b>Arrivals</b>: ' + [f'{x:,.0f}' for x in airport3['cnt']] + '<br>'
